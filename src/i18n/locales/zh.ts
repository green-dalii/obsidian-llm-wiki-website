...
export const zh: Translations = {
  ...
  hero: {
    ...
    subtitle: '在 Obsidian 记录——笔记、PDF、网页剪藏。每一种输入都连接成一张不断生长的网络。',
    ctaInstall: '获取插件',
  ...
  features: {
    ...
    pdfIngestDesc: '把 PDF（包括扫描件）放进 sources/。AI 逐字转成 Markdown，标记图表与公式，接入同一套实体、概念与链接提取流程。',
    pdfIngestTag: 'PDF 摄入',
    moreLabel: '更多能力：',
  ...
  faq: {
    ...
    items: [
      ...
      { q: '插件能读取 PDF 和扫描文档吗？', a: '可以——从 v1.25.0 起，PDF 是正式支持的来源格式。将 PDF 放入 sources/ 文件夹：插件会通过你的 LLM 提供商读取它（Anthropic Claude、OpenAI GPT-4o 和 Bedrock 系列都原生支持 PDF），再通过带有防幻觉图表与公式标记的 OCR 风格逐字转写器将其转换为 Markdown，随后与 Markdown 来源一样，进入实体、概念和链接提取流程。转换后的 Markdown 会按内容哈希缓存在 <em>.obsidian/</em> 中，因此重新摄入会立即完成。默认情况下不会修改你的库；在设置中启用 <em>Write PDF Markdown to Vault</em> 后，还会在源 PDF 旁写入 <em>&lt;basename&gt;.pdf.md</em> 配套文件。' },
