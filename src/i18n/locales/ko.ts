...
export const ko: Translations = {
  ...
  features: {
    ...
    pdfIngestDesc: '논문, 매뉴얼, 스캔한 영수증도 sources/에 넣으세요. 텍스트, 표, 그림을 추출하고, 읽은 페이지까지 기록합니다.',
    pdfIngestTag: 'PDF 수집',
    moreLabel: '그리고 더:',
  ...
  faq: {
    ...
    items: [
      ...
      { q: '플러그인이 PDF와 스캔 문서도 읽을 수 있나요?', a: '네 — v1.25.0부터 PDF는 정식 소스 형식이 되었습니다. PDF를 sources/ 폴더에 넣으면, 플러그인이 LLM 제공업체(Anthropic Claude, OpenAI GPT-4o, Bedrock 계열은 PDF를 네이티브 처리)를 거쳐 읽고, OCR 방식의 축어 변환기로 그림과 수식에 환각 방지 마커를 붙여 마크다운으로 변환한 뒤, 같은 개체 / 개념 / 링크 추출 파이프라인으로 보냅니다. 변환된 마크다운은 <em>.obsidian/</em> 안에 콘텐츠 해시로 캐시되어 재수집은 즉시 끝납니다. 기본적으로 보관함은 변경되지 않습니다. 설정에서 <em>PDF 마크다운을 보관함에 쓰기</em>를 켜면 소스 PDF 옆에 <em>&lt;basename&gt;.pdf.md</em> 사이드카 파일도 함께 저장됩니다.' },
