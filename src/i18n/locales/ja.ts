...
export const ja: Translations = {
  ...
  faq: {
    ...
    items: [
      ...
      { q: 'プラグインは私のノートを変更しますか？AI が間違ったことをしたらどうなりますか？', a: 'あなたの Vault は書かれたままの状態を保ちます。プラグインは独自のフォルダ（entities/、concepts/ など）に新しい Wiki ページを作成するだけで、これらはすべて wiki/ フォルダの下に作られます。自動生成されたページが間違っている場合は、削除してインジェストを再実行するだけで、最初から再構築されます。一度レビュー済みとしてマークすると、AI はそのページを書き換えず、追記のみを行います。すべての会話形式の回答も、元となったノートにリンクされているため、ワンクリックで主張を検証できます。' },
