...
**自动化页面创建。** 你正常地在 Vault 里任意位置写笔记。按 `Cmd+P` → "Ingest Sources" 时，LLM Wiki 读取每个文件，识别其中的实体和概念，为每一个创建专属页面。由 LLM 来判断什么值得建页面——不是你。它能够准确提取人物、理论、项目和技术术语，因为它理解文本的含义。
...
1. 你找到一篇关于检索增强生成的有趣文章。把要点复制到 Vault 里任意位置的一个新笔记。花 2–3 分钟。
2. 按 `Cmd+P`，输入 "Ingest single source"，回车。
...
**从来源到结构化 Wiki，只需 15 秒。** 这就是核心循环：写，提取，探索。
