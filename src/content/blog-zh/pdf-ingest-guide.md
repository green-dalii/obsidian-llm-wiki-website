...
v1.27.0 还修了一个不那么显眼、但你打开来源页时会立刻注意到的地方。每张实体页和概念页早就带有一段 `Mentions in Source`——用抽取阶段捕获的原文引用搭出来的。但代表原始文档本身的那张页——`sources/<slug>.md`——一直没有。以前你想验证某条说法，从 Wiki 页跳到来源页，证据链就在来源页断了。

现在不会断了。转写出的来源页带着和每张实体页一样的原文引用链路，通回底层文档。Lint 还会把每条引用回到底层 PDF（或 Markdown）做校对，让错放的引用在进入 Wiki 之前就被拦下。对研究工作流而言，从 Wiki 任何一页都能直接走到原始文档，反过来也对得上。
